...
const HIGHTLIGHT_COLOR_EXAMPLE = 'blue'
...
const colorizeStrTmpl = (color, str) => 
	`<span class="emphasize" style="color:${color}">${str}</span>`
...
function colorizeHanziExample(hanziTmpl) {
	if (!example) {
  	exampleElt.innerHTML = '<span class="no-example">No example provided</span>'
  }
	else if (example.includes(hanzi)) {
    const tmpl = example.replace(hanzi, hanziTmpl)
    exampleElt.innerHTML = tmpl
...
function getColorizedPinyinTmpl() {
  ...
  var pinyinTmpl = '', hanziTmpl = ''
    ...
    for (const toneColor in TONE_COLOR_TO_VOWELS) {
    	if (TONE_COLOR_TO_VOWELS[toneColor].includes(curChar)) {
      	// tone vowel exists in the word
      	pinyinTmpl += colorizeStrTmpl(toneColor, curChar) // map tone color to vowel
        if (!hanzi[hanziIdx]) {
        	// idx of the hanzi does not exists
...
try {
	const { pinyinTmpl, hanziTmpl } = getColorizedPinyinTmpl()
  hanziElt.innerHTML = hanziTmpl
	pinyinElt.innerHTML = pinyinTmpl
  colorizeHanziExample(hanziTmpl)
} catch (err) {
